						...
						<tr>
							...
							<?php
									for ($i=1; $i <=10 ; $i++) { 
										...
										if(($this->vkpAccaunts[$i]*1)>0){
											// делаем запрос для пользователя
											if($this->vkpAccaunts_type[$i]=='user'){
												...
												if(is_array($resp['error'])){
													...
													$vk_name = "<a href='http://vk.com/".($this->vkpAccaunts_type[$i]=='group' ? "club":"id").$this->vkpAccaunts[$i]."' target='_blank'><font color='green'>".$resp['response'][0]['first_name']." ".$resp['response'][0]['last_name']."</font></a>";
												}
											}
											// запрос для группы
											 if($this->vkpAccaunts_type[$i]=='group'){
										...
										echo "\n<tr><td>#".$i."</td><td>\n";                                        
										echo "<input type='number' style='width:160px;' name='vkpAccaunts[".$i."]' value='".$this->vkpAccaunts[$i]."'>\n";
										echo "<input type='radio' name='vkpAccaunts_type[".$i."]' value='group'".($this->vkpAccaunts_type[$i]=='group' ? " checked" : "")."> ".__('group','vkp')."\n"; 
										echo "<input type='radio' name='vkpAccaunts_type[".$i."]' value='user'".($this->vkpAccaunts_type[$i]=='user' ? " checked" : "")."> ".__('user','vkp')."\n";
										echo "&nbsp;&nbsp;".$vk_name."</td><tr>";
									...
									for ($i=1; $i <=11 ; $i++) { 
										...
										if(($this->vkpAccaunts[$i]*1)>0){
											...
											if($_error==0){
												...
												$resp = $this->VKP->api('photos.getAlbums', array('access_token'=> $this->vkpAccessToken,'owner_id'=>($this->vkpAccaunts_type[$i]=='group' ? "-":"").$this->vkpAccaunts[$i]));
